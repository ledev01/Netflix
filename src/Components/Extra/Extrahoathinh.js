...
import { fetchFilmsHoatHinh } from "../../Services/itemService";
import { useEffect, useState, useCallback, useMemo } from "react";
import { Link } from 'react-router-dom';
import { LazyLoadImage } from 'react-lazy-load-image-component';
import 'react-lazy-load-image-component/src/effects/blur.css';

function Extrahoathinh({ showPagination = true }) {
  const [filmsHoatHinh, setFilmsHoatHinh] = useState([]); // State to store the filtered list of films
    ...
    <div className="sm:w-[860px] w-full h-auto mr-auto mt-10 flex sm:justify-start flex-col justify-center">
      <div className="flex flex-col">
        <div className="flex flex-row">
          ...
          <a href="/hoathinh" className="text-white font-light bg-gradient-to-r from-orange-400 to-pink-600 rounded-2xl px-5 ml-auto flex items-center">
              Xem Thêm
            </a>
        </div>
        ...
        <div className="grid sm:grid-cols-4 grid-cols-2 mx-auto sm:w-[860px] sm:gap-x-4 sm:gap-y-4 gap-x-3 gap-y-3 mt-3">
          {Array.isArray(filmsHoatHinh) && filmsHoatHinh.length > 0 ? (
            filmsHoatHinh.map((item) => (
              ...
              <div className="sm:w-[201.66px] w-[170px] h-[340px] bg-gray-800 flex flex-col rounded-md overflow-hidden relative">
                ...
                {/* <a className="absolute right-2 top-[252px] w-[67.43px] h-6 z-10 rounded-tl-md rounded-br-md rounded-tr bg-gradient-to-r from-red-500 to-yellow-700 text-white text-xs font-semibold flex items-center justify-center">Hoàn tất</a> */}
