...
import { fetchFilmsTvShows } from "../../Services/itemService";
import { useEffect, useState, useCallback, useMemo } from "react";
import { Link } from 'react-router-dom';
import { LazyLoadImage } from 'react-lazy-load-image-component';
import 'react-lazy-load-image-component/src/effects/blur.css';

function Extratvshow({showPagination =true}) {
  ...
  const getFilm = async (page) => {
    try {
      let res = await fetchFilmsTvShows(page); // Pass the current page to the API
    ...
    <div className="sm:w-[860px] w-full h-auto mr-auto mt-10 flex sm:justify-start flex-col justify-center">
      <div className="flex flex-col">
        <div className="flex flex-row">
          ...
          <a href="/tvshow" className="text-white font-light bg-gradient-to-r from-orange-400 to-pink-600 rounded-2xl px-5 ml-auto flex items-center">
            Xem Thêm
          </a>
        </div>
