...
function Extraphimle({ showPagination = true }) {
  const [filmsLe, setFilmLe] = useState([]); // State to store the filtered list of films
  ...
  const pageNumbers = useMemo(() => {
    return Array.from({ length: 5 }, (_, i) => i + 1); // [1, 2, 3, ..., totalPages]
  }, []);

  // Fetch films when the component mounts or the active page changes
  useEffect(() => {
    getFilm(activePage);
  }, [activePage]);
      ...
      <div className="flex flex-col ">
       <div className=" ">
          <div className="flex flex-row ">
            ...
            <a href="/phimle" className="text-white font-light bg-gradient-to-r from-orange-400 to-pink-600 rounded-2xl px-5 ml-auto flex items-center">
              Xem Thêm
            </a>
          </div>
        ...
        <div className="grid sm:grid-cols-4 grid-cols-2 mx-auto sm:w-[860px] sm:gap-x-4 sm:gap-y-4 gap-x-3 gap-y-3 mt-3">
          {Array.isArray(filmsLe) && filmsLe.length > 0 ? (
            filmsLe.map((item) => (
              <Link key={item._id} to={`/trangchitietphim/${item.slug}`}>
                <div className="sm:w-[201.66px] w-[170px] h-[340px] bg-gray-800 flex flex-col rounded-md overflow-hidden relative">
                 ...
                 {/* <span className=" lazy-load-image-background blur lazy-load-image-loaded transition-delay: 0.2s;" > <img className="w-[201.66px] h-[281.5px] rounded-tl-md rounded-tr-md transform transition-transform duration-200 hover:scale-105 object-cover" src={`https://img.ophim.live/uploads/movies/${item.thumb_url}`}/></span> */}
                 <span className="relative">
                  ...
                  <div className="sm:w-[201.66px] w-[170px] flex flex-col bg-gray-800 justify-center items-center z-10">
                    <div className="flex justify-center sm:w-[185.66px] w-[160px]">
                      <a to={`/trangchitietphim/${item.slug}`} className="text-yellow-300 font-semibold truncate ">{item.origin_name}</a>
                    </div>
                    <div className="flex justify-center sm:w-[185.66px] w-[160px]">
